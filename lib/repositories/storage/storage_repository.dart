...
class StorageRepository extends BaseStorageRepository{
  ...
  // 'image' is the image file we want to upload
  // 'ref'  is the path of the image we want to upload
  ...
  Future<String> uploadProfileImage({@required  String url, @required  File image}) async{
    var imageId = Uuid().v4();  // Generates unique IDs
  ...
  Future<String> uploadPostImage({@required  File image}) async{
    final imageId = Uuid().v4();  // Generates unique IDs
    final downloadUrl = await _uploadImage(image: image, ref: 'images/posts/post_$imageId.jpg');
